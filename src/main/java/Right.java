...
/*Moves player Right */
public class Right extends MovementDecorator {

    /* Logger is created */
    private static final Logger LOG = Logger.getLogger(Right.class.getName());

    /* Decorator */
    public Right(Movement next) {
        super(next);
    }
    ...
    public boolean move(char[][] array, int row, int col) throws CustomException {
        ...
        /* If player is in the right exception thrown */
        if (array[rowV][colV] == array[rowV][col - 1]) {
            throw new CustomException("Cant move player out of maze");
        }

        /*
         * Checks if the element to the right is a space or a wall and allows the user
         * to move
         * if it is a space
         */
